...
output_configure_line()
{
    ...
    echo
    echo "cmake ../src -DSCIRUN_THIRDPARTY_DIR=$full_dir"
    echo
    echo "----------------------------------------------------------"
    echo
}
...
setup_num_bits_flags()
{
    ...
    # in case we need to overrule default setting
    if test "$use_64" = "64"; then
       BITS="64"
    fi
    echo "Bits: $BITS."
    echo "OS:   $osname."
...
parse_args()
{
    ...
    if test "$1" = "--64bit"; then
       echo "Building a 64bit build of the thirdparty software"
       use_64="64"
       shift 1
    fi

    if test "$1" = "--static-only"; then
        echo "Building static only build"
        shared_libs="DISABLE"
        remove_shared_libs=1
        shift 1
    fi

    if test "$1" = "--verify-install"; then
...
check_for_libxml2()
{
	...
	echo 
	echo "Ok, I will build it for you."
    fi
...
add_freetype_packages()
{
    ...
    if test "$shared_libs" = "DEFAULT"; then
        FREETYPE_LIST="$FREETYPE_LIST libfreetype.$SH_LIB_EXT"
    fi
}
...
add_tcl_packages()
{
    TCL_LIST="PACKAGE tcl                                \
                           ...
                           LIBS         libtclstub8.3.a tcl tcl8.3 tclConfig.sh"
    if test "$shared_libs" = "DEFAULT"; then
        TCL_LIST="$TCL_LIST libtcl.$SH_LIB_EXT libtcl8.3.$SH_LIB_EXT"
    else
        TCL_LIST="$TCL_LIST libtcl8.3.a"
    fi
}
...
add_tk_packages()
{
    TK_LIST="PACKAGE tk                                 \
                          ...
                          LIBS         libtkstub8.3.a tk tk8.3 tkConfig.sh"
    if test "$shared_libs" = "DEFAULT"; then
        TK_LIST="$TK_LIST libtk.$SH_LIB_EXT libtk8.3.$SH_LIB_EXT"
    fi
}
...
add_itcl_packages()
{
    ITCL_LIST="PACKAGE itcl                               \
                            ...
                            LIBS         itcl itcl3.1 itclConfig.sh itk iwidgets iwidgets2.2.0 iwidgets3.1.0  \
                                         libitclstub3.1.a libitkstub3.1.a"
    if test "$shared_libs" = "DEFAULT"; then
        ITCL_LIST="$ITCL_LIST libitcl.$SH_LIB_EXT libitcl3.1.$SH_LIB_EXT libitk.$SH_LIB_EXT"
    else
        ITCL_LIST="$ITCL_LIST libitcl3.1.a libitk3.1.a"
    fi
}
...
add_blt_packages()
{
    BLT_LIST="PACKAGE blt                                \
                            ...
                            LIBS         blt2.4 libBLT.a libBLT24.a libBLTlite.a libBLTlite24.a"
    if test "$shared_libs" = "DEFAULT"; then
        BLT_LIST="$BLT_LIST libBLT.$SH_LIB_EXT libBLT24.$SH_LIB_EXT libBLTlite24.$SH_LIB_EXT"
    fi
}
...
add_png_packages()
{
    PNG_LIST="PACKAGE png                               \
                            ...
                            LIBS         libpng.a"
    if test "$shared_libs" = "DEFAULT"; then
        PNG_LIST="$PNG_LIST libpng.$SH_LIB_EXT"
    fi
}
...
#     echo "verification_only = $verification_only"
#     if [[ -n "$verification_only" ]]; then
#         echo "verification_only on"
#     else
#         echo "verification_only off"
#     fi
    ...
    if test "$all_passed" = "no"; then
        ...
        if test "$STAGE" = "Initial verification" && test -d $full_dir/src/teem-1.9.0-src && [[ -z "$verification_only" ]]; then
...
#
# Adding -static flags to the build system to make it only look for .a
# files would work in an ideal world.  However on some systems like
# Mac the linker will *always* prefer the shared library version if it
# is present.  Not only that, unless you specify
# -Wl,-search_paths_first the linker will search the paths depth first
# rather than breadth first (like most other systems do).
#
# To get static linkage to work properly on the Mac you cannot have
# static and dynmaic libraries living in the same directory.
if [[ -n "$remove_shared_libs" ]]; then
    cd ${full_dir}/lib/
    rm -rf *.${SH_LIB_EXT}*
fi
