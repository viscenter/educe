...
template <class FIELD> 
MesquiteMesh<FIELD>::MesquiteMesh( ProgressReporter* pr, FieldHandle field, 
     std::string isomethod, double isovalue)
{
  ...
  if (field_->basis_order() == 0)
  {
    ...
    typename FIELD::value_type isoval(isovalue);
    mesh_->begin(it);
    mesh_->end(eit);
    ...
    {    
      while (it != eit)
      {
        ...
        if (isoval < val)
        {
          ...
          {
            fixed_[static_cast<unsigned int>(nodes[p])] = true;
          }
        }
        ++it;
      }
  ...
  else if (field_->basis_order() == 1)
  {
    ...
    if (isomethod == "GreaterThan")
    {
      while (it != eit)
      {
        ...
        if (isoval > val)
        {
          fixed_[static_cast<unsigned int>(*it)] = true;
        }
        ++it;
      }        
    }
  ...
  typename FIELD::mesh_type::Elem::index_type nit;
  typename FIELD::mesh_type::DElem::array_type delems;
  typename FIELD::mesh_type::Node::array_type nodes; 
  ...
  const TypeDescription *mtd = mesh_->get_type_description();
