...
class ScrollUnwrapPlugin : public UnwrapPlugin {
	public:
		...
		BaseTool::propagation_state_e process_event(event_handle_t event) /*const*/ {
			...
			if (dynamic_cast<FinishEvent *>(event.get_rep())) {
				if (!painter_->check_for_active_data_volume("Unwrap Section filter"))
				{
					return STOP_E;
				}
				else if(!(painter_->mask_volume_.get_rep())) {
					painter_->set_status("Unwrap Section filter requires a mask to be selected.");
					return STOP_E;
				}
	...
	private:
		...
		virtual void run_filter() {	
			printf("Unwrap Section run\n");

			painter_->set_status("Running section unwrapping filter.");
