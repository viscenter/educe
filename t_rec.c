...
int main( int argc, char *argv[] )
{
	...
	if(isOpenSuc == 1)
	{
		sprintf(ofnamext, "%s.vol", volfileNameStr);
	...
	/* Open the vol file, write header data */
	int headerLength;
	char endSliceStr[100], countStr[30],startSliceStr[100], extraInfoStr[10];
	char stateValidFlg;
	FILE *ffp;
	C_IntTuple volumeSize;

	float xRotateAngle = 0.0;
	float yRotateAngle = 0.0;
	float xzoom = 1.0;
	float yzoom = 1.0;
	float zzoom = 1.0;
	int corex = 1, corey = 1;
	...
	int radius = 0;
	char *voxelState;
	int pixelsPerVolume, sliceThisTransaction;
	int headerEnd;

	volumeSize.x = ImageSize;
        volumeSize.y = ImageSize;
        volumeSize.z = sliceNum + slicesSent;

        pixelsPerVolume = volumeSize.x*volumeSize.y*volumeSize.z;
	stateValidFlg='Y';
	sprintf(countStr,"%dx%dx%d",ImageSize, ImageSize, sliceNum + slicesSent);
	sprintf(endSliceStr, "Volume Size: %s\n", countStr);
	sprintf(extraInfoStr, " ");
	sprintf(startSliceStr, "Start Slice:\ne:\\output\\%s001.asc\n",volfileNameStr);
	headerLength = strlen(endSliceStr)+1+strlen(startSliceStr)+1+strlen(extraInfoStr)+1;
	headerLength++;

        //open the vol file and write the file header info into it.
        sprintf(ofnamext,"%s.vol", volfileNameStr);
        ...
        fwrite(&headerLength, sizeof(int), 1, ffp);
        fwrite(startSliceStr, sizeof(char), strlen(startSliceStr)+1, ffp);
        fwrite(endSliceStr, sizeof(char), strlen(endSliceStr)+1, ffp);
        fwrite(extraInfoStr, sizeof(char), strlen(extraInfoStr)+1, ffp);
        fwrite(&stateValidFlg, sizeof(char), 1, ffp);//state part flg 

        fwrite(&volumeSize, sizeof(int), 3, ffp);
        fwrite(&xRotateAngle, sizeof(float), 1, ffp);
        fwrite(&yRotateAngle, sizeof(float), 1, ffp);
        fwrite(&xzoom, sizeof(float), 1, ffp);
        fwrite(&yzoom, sizeof(float), 1, ffp);
        fwrite(&zzoom, sizeof(float), 1, ffp);

        fwrite(&corex, sizeof(int), 1, ffp);
        fwrite(&corey, sizeof(int), 1, ffp);
        fwrite(&radius, sizeof(int), 1, ffp);
        printf("vol file header was wrote!\n\n");

	headerEnd = (int)ftell(ffp);
	...
	while (outstandingProcesses > 0)
	{
	    ...
	    if (fseek(ffp, fileOffset[0] * sizeof(float) + headerEnd, SEEK_SET) == -1)
	...
	// write the tail of vol file.
	if(fseek(ffp, 0, SEEK_END) == -1)
	    perror("Error seeking");
	voxelState = (char *)calloc(pixelsPerVolume, sizeof(char));
	if(stateValidFlg=='Y')
	    fwrite(voxelState, sizeof(char), pixelsPerVolume, ffp);
	if (voxelState != NULL) {free(voxelState); voxelState=NULL;}
...
void getTenSlices(char * filename, int jumpline, float *slices, float *MPOSSptr)
{
	...
	float xmin, xmax, ymin, ymax, zmin, zmax, readnum;
	...
	else
	{
		...
		for (sinoFileCount = 10; sinoFileCount > 0; sinoFileCount--)
		{
			...
			for (j = 0; j < step; j++)
			{
				for (k = 0; k < width; k++)
				{
					if (fscanf(fp, "%f", &readnum) != 1)
					    printf("\nError reading file");
					*sliceptr = readnum / zmax;
					sliceptr++;
				}
				if (j == jumpline - 1) 
				 	sliceptr -= step*width;
